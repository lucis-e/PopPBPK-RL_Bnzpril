...
file.close()
data_ = [list(x) for x in set(tuple(x) for x in data)]   # Lista única de experiencias porque solo necesitamos la información 
...
path_qtables = '../Results/Train/HyperparameterTuning/'  # tabla-Q de la función de utilidad


q_tables = {}    # almacena las tablas-Q con un identificador con la información de los hiperparámetros del modelo

for file in os.listdir(path_qtables):
    if (file.endswith('.pickle')) and (file.startswith('qtable')):
        param = file.split(sep = '_')[1:3]  # identificador 
        name = param[0]+ '_' + param[1]
        with open(path_qtables+file, 'rb') as fil:
            q_tables[name] = pickle.load(fil)
...
for key in qtables_names:
    ...
    for index, row in antropometric_data.iterrows():    # Itera sobre todos los pacientes -> para hacerlo lo más parecido al módulo PBPK voy a llamar a la función desde aquí
        ...
        score = 0  
        print(f'Patient: p{index+1}   Hyperparameter combination (alfa_gamma): {key}')
...
mean_cr_df.to_excel('../Results/Train/HyperparameterTuning/AVGr_by_HPcombination.xlsx')  # Guarda los valorespara cada combinación de parámetros siguiendo la poítica óptima aprendida durante el entrenamiento
